...
public class AlunoService implements AlunoRepository {
    ...
    public void cadastrarAluno(Aluno aluno) throws CadastroExistenteException, SQLException {
        try (Connection conn = DataBaseConnection.getConnection()) {//Conecta ao banco de dados
            if (pesquisarAluno(aluno.getCpf(), conn) != null) {
            ...
            String insert = "INSERT INTO aluno(nome, cpf, telefone, data_nascimento, rua, bairro, cidade, estado, quantidade_sessoes, preco_por_hora, total_a_pagar) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void excluirAluno(Aluno aluno) throws AlunoNaoEncontradoException, SQLException {
        try (Connection conn = DataBaseConnection.getConnection()) {//Conecta ao banco de dados
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(query)) { // Prepara a Consulta ao banco
                pstmt.setString(1, aluno.getCpf()); // Define o cpf do aluno a ser excluido.
                int rowsAffected = pstmt.executeUpdate();// Executa a consulta e verifica se o aluno foi excluído com sucesso
                if (rowsAffected == 0) {
    ...
    @Override
    public Aluno pesquisarAluno(String cpf, Connection conn) throws SQLException {
        String query = "SELECT * FROM aluno WHERE cpf = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(query)) { //Prepara a consulta
